function  [Outcome] = CMMF15(PopDec) 
                M=2;
                OptX = 0.2;
                [N,D]  = size(PopDec);
                Pop=PopDec;
    ...
    obj.h = zeros(size(PopDec,1),1);
                ...
                for i=1:N
                    if Pop(i,1)>=0
                        ...
                        T_(i) = (1 - (Pop(i,1)+1).^2-Pop(i,2).^2).^2 + obj.h(i);
                        G_(i,1:M) = [ones(1,1) cumprod(sin(pi/2*obj.THETA_(i)),2)] .* [cos(pi/2*obj.THETA_(i)) ones(1,1)];
                    end
                end
...
%% PopCon     
                ...
                for i=1:N
                    if Pop(i,1)>=0   
                        ...
                        PopCon(i,3)=Pop(i,1).^2+4*Pop(i,2).^2-1;
                        PopCon(i,4)=-Pop(i,1).^2-3*Pop(i,2).^2+3/4;
                    elseif Pop(i,1)<0  
                        PopCon(i,1)= obj.THETA_(i,1)-2/3;
                        PopCon(i,2)=-obj.THETA_(i,1)+1/3;
                        PopCon(i,3)=(Pop(i,1)+1).^2+4*Pop(i,2).^2-1;
                        PopCon(i,4)=-(Pop(i,1)+1).^2-3*Pop(i,2).^2+3/4;
                    end
                end
             PopCon(PopCon <= 0) = 0;
             PopCon = abs(PopCon);

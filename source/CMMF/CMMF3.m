...
if D>2
    ...
    obj.h = zeros(size(PopDec,1),1);
            ...
            G_=zeros(N,M);
                for i=1:N
                    if Pop(i,1)<0&&Pop(i,1)>-1/2
                        T_(i) = (-0.96 - Pop(i,1)-Pop(i,2)).^2 + obj.h(i);
                        G_(i,1:M) = 1-[ones(1,1) cumprod(sin(pi/2*obj.THETA_(i)),2)] .* [cos(pi/2*obj.THETA_(i)) ones(1,1)];
                    elseif Pop(i,1)<=-1/2
                        T_(i) = (-0.96 - Pop(i,1)-Pop(i,2)).^2 + obj.h(i);
                        G_(i,1:M) = 1-[ones(1,1) cumprod(sin(pi/2*obj.THETA_(i)),2)] .* [cos(pi/2*obj.THETA_(i)) ones(1,1)];
                    else
                        T_(i) = (0.81 - PopDec(i,1)-PopDec(i,2)).^2 + obj.h(i);
                        G_(i,1:M) = 1-[ones(1,1) cumprod(sin(pi/2*obj.THETA_(i)),2)] .* [cos(pi/2*obj.THETA_(i)) ones(1,1)];
                    end
                end
             ...
             for i=1:N
                 if Pop(i,1)<0&&Pop(i,2)>=0    
                     PopCon(i,1)= -obj.Sx(i,1);
                     PopCon(i,2)= obj.Sx(i,1)-0.25;
                     PopCon(i,3)= 0;
                 elseif Pop(i,1)<=0&&Pop(i,2)<=0 
                     PopCon(i,1)= Pop(i,1)+Pop(i,2)+0.9;
                     PopCon(i,2)= -Pop(i,1)-Pop(i,2)-1;
                     PopCon(i,3)= Pop(i,1)+1/2;
                 elseif Pop(i,1)>=0&&Pop(i,2)>=0   
                     PopCon(i,1)= -(obj.THETA_(i,1)-b);
                     PopCon(i,2)= Pop(i,1)+Pop(i,2)-1;
                     PopCon(i,3)= -Pop(i,1)-Pop(i,2)+0.64;
                 else    
                     PopCon(i,1)= -obj.Sx(i,1);
